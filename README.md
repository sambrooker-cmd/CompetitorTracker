...
Designed to run on free tiers wherever possible: a free hosted Postgres
(Neon/Supabase), a free Node host for the API, and a free static host for
the dashboard. The one thing that can push this past "fully free" is
scraping a JavaScript-rendered competitor site — see **Static vs JS-rendered
competitors** below.
...
- **`server/`** — Express + TypeScript API, backed by Prisma/Postgres.
...
Requires Node 20+ and a Postgres database — either a local install, or a
free [Neon](https://neon.tech)/[Supabase](https://supabase.com) project
(a separate branch/project from whatever you use for production works
well, so local work never touches real data).

```bash
npm install                     # installs both workspaces

# --- server ---
cp server/.env.example server/.env   # set DATABASE_URL to your local or Neon/Supabase Postgres
cd server
...
**Database — a free hosted Postgres**, e.g. [Neon](https://neon.tech) or
[Supabase](https://supabase.com) free tier. Create a project, copy the
connection string it gives you — that's `DATABASE_URL` for the API
deployment below. (Don't use SQLite in production: most free web-service
tiers, including Render's, have an ephemeral filesystem that wipes a
SQLite file on every restart/redeploy.)

**API — a free Node host**, e.g. [Render](https://render.com) free web
service or [Railway](https://railway.app) free tier. The repo includes a
`render.yaml` Blueprint, so on Render: dashboard → **New** → **Blueprint**
→ point at this repo → it reads the file and creates the service. You
still need to fill in two dashboard fields it deliberately leaves blank
(they're specific to your setup, not something a template should guess):
`DATABASE_URL` (from the step above) and `CORS_ORIGIN` (your deployed
frontend's URL, once you have it — circle back after the frontend step
below). `SCRAPE_TRIGGER_TOKEN` is auto-generated by the blueprint.

Without the blueprint (Railway, or Render configured by hand), set the
same env vars from `server/.env.example` yourself, with root directory
...
3. Client-side routing needs a rewrite fallback so refreshing a page like
   `/competitors` doesn't 404 on a static host — already handled:
   `client/vercel.json` for Vercel, `client/public/_redirects` for
   Netlify (copied into the build output automatically).

Once the frontend is deployed, go back and set the API's `CORS_ORIGIN` to
its URL.
